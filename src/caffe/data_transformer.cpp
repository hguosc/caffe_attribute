...
void DataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
  ...
  cv::resize(cv_cropped_img, cv_resize_img, cv::Size(width2, height2));
  Dtype* transformed_data2 = transformed_blob2->mutable_cpu_data();
  int top_index2;
  for (int h = 0; h < height2; ++h) {
    const uchar* ptr = cv_resize_img.ptr<uchar>(h);
    int img_index = 0;
    for (int w = 0; w < width2; ++w) {
      for (int c = 0; c < img_channels; ++c) {
        if (do_mirror) {
          top_index2 = (c * height2 + h) * width2 + (width2 - 1 - w);
        } else {
          top_index2 = (c * height2 + h) * width2 + w;
        }
        // int top_index2 = (c * height2 + h) * width2 + w;
        Dtype pixel = static_cast<Dtype>(ptr[img_index++]);
        // mean is not appliable here
        if (has_mean_file) {
          int mean_index = (c * img_height + h_off + h) * img_width + w_off + w;
          transformed_data2[top_index2] =
            (pixel - mean[mean_index]) * scale;
        } else {
          if (has_mean_values) {
            transformed_data2[top_index2] =
            ...
            transformed_data2[top_index2] = pixel * scale;
          }
        }
      }
    }
