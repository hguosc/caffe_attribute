...
void ContrastiveLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    if (dist_sq_.cpu_data()[i] > max_dist)
      max_dist = dist_sq_.cpu_data()[i];
    if (dist_sq_.cpu_data()[i] < min_dist)
      min_dist = dist_sq_.cpu_data()[i];
  ...
  // LOG(INFO) << "=============>>>>>>>>>d(max, min) = (" << max_dist << ", "<< min_dist <<")";
